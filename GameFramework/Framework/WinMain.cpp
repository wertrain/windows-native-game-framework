...
DWORD WINAPI GameMainFunc(LPVOID vdParam)
{
	...
	while( IsWindow( gameWindow->hWnd ) )
	{
		...
		// 理想時間の算出
		idealTime = (DWORD)( frames * ( 1000.0F / gameWindow->dwFps ) );
		...
		BitBlt(hdc, 0, 0,
			...
			0, 0,
			SRCCOPY);
		PatBlt(gameWindow->hScreenDC,0,0,gameWindow->size.cx,gameWindow->size.cy,BLACKNESS); // 裏画面を白で初期化
		...
		if(progress >= 1000){
			wsprintf(str, TEXT("FPS %d\0"), frames);
			beforeTime = nowTime;
			frames = 0;
		}
		++frames;
